...
- [ ] E-15: AUDIT-P0: state-db.js singleton ignores aiDir on subsequent calls — silent bug if two consumers in same process pass different aiDir. Key cache by aiDir or assert mismatch. | Tier: 2
...
- [ ] E-18: AUDIT-P1: Unify structured logging across all 21 MCP servers. Currently only 4/21 emit structured JSON (advisor, approval, cache-manager, computer-use). Extract shared src/shared/logger.js helper and migrate. | Tier: 2
...
- [ ] E-20: AUDIT-P1: Hoist @modelcontextprotocol/sdk version pin to root package.json — child workspaces all use ^1.0.0 caret, allowing minor drift between servers. Pin exact version at root, set children to "*". | Tier: 1
...
- [ ] E-27: AUDIT-P2: Replace string-search tests (grep / String.includes against source) with stdio JSON-RPC roundtrip behavioral tests. Current tests broke on cosmetic single/double quote differences. Build a tests/lib/mcp-client.sh helper. | Tier: 2
...
- [ ] E-29: AUDIT-P3: archive-manager-mcp hardcodes ~/.ai-os/bin/ai path. Add ENOENT fallback or accept ai-bin path arg for users with custom install prefixes. | Tier: 1
