...
2. **Launch-time role for the SessionStart hook** — `session-start.sh` mints the token for
   ...
   `additionalContext` so the persona layer can read the minted role.
3. **Per-role settings overlay `.claude/settings.<role>.json`** (generated by `ai init`/`ai sync`
   from `src/claude/`, one per role that maps to `claude`) — carries only `env.AI_OS_CALLER_ROLE`
   and role-specific permission deltas. It MUST NOT register a second `SessionStart` hook
