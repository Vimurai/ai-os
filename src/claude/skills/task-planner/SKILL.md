...
- P-## for Architect tasks
- E-## for Engineer tasks
...
Append under the correct section (`## Engineer` or `## Architect`).
...
Why the shell command rather than `mcp__task-synchronizer-mcp__handoff_control`:
`ai handoff` is a plain shell command that works even when project MCP servers are
unavailable to the session, so it is the reliable primitive. It writes the
