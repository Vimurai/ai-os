...
assert_contains "T-1b: template defaults architect=gemini:1 engineer=claude:0" "gemini,1,claude,0" "$tmpl"
...
# T-5: _write_roles_json falls back to legacy defaults when no flags set
tmp_def="$(mktemp -d)"
...
assert_contains "T-5: default architect=gemini:1 engineer=claude:0" "gemini,1,claude,0" "$def"
