...
# E-180 (D-049): REJECTED joined the enum so expected rejections book a distinct usage-friction
# status instead of polluting ERROR (or hiding in SUCCESS, as E-179 did).
assert_status 0 "CHECK constraint on status column (SUCCESS/ERROR/TIMEOUT/REJECTED)" \
  grep -qE "status IN \('SUCCESS','ERROR','TIMEOUT','REJECTED'\)" "$TELEMETRY"
